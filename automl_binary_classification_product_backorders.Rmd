...
## convert h20frame object to dataframe
# as.data.frame(df) %>%
#   head()
...
```{r}
y <- "went_on_backorder"
x <- setdiff(names(df), c(y, "sku")) ## removes "went_on_backorder" and "sku" from vector of predictor col names
```
...
## # Get the "All Models" Stacked Ensemble model (tidyverse)
se <- model_ids %>%
  filter(str_detect(model_id, "StackedEnsemble_AllModels")) %>%
  pull(model_id) %>%
  h2o.getModel()
...
h2o.varimp(metalearner) %>%
  as.data.frame() %>%
  ggplot(aes(relative_importance, reorder(variable, relative_importance))) +
  geom_col(aes(fill = variable), show.legend = FALSE) +
  labs(y = "Model", x = "Relative Importance") +

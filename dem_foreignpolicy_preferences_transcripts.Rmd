...
one_hot_train_labels <- candidates_train %>%
    select(speaker) %>%
    mutate(speaker = as.numeric(as.factor(speaker)) -1) %>%
    pull(speaker) %>%
    to_categorical()

one_hot_test_labels <- candidates_test %>%
    select(speaker) %>%
    mutate(speaker = as.numeric(as.factor(speaker)) -1) %>%
    pull(speaker) %>%
    to_categorical()
...
prepped_recipe <- recipe( ~ transcript, data = candidates_train) %>%
    step_tokenize(transcript) %>%
    #step_stopwords(transcript) %>%
    #step_stopwords(transcript, custom_stopword_source = custom_stopwords) %>% 
    #step_ngram(transcript, num_tokens = 2, min_num_tokens = 1) %>%
    step_tokenfilter(transcript, max_tokens = max_words) %>%
    #themis::step_upsample(speaker) %>%
    step_sequence_onehot(transcript, sequence_length = max_length,
                         prefix = "", padding = "post", truncating = "post") %>%
    prep()
...
results <- dense_model %>%
    evaluate(bake(prepped_recipe, candidates_test, composition = "matrix"),
             one_hot_test_labels)

colnames_keras <- candidates_train %>%
    select(speaker) %>%
    mutate(speaker2 = as.numeric(as.factor(speaker)) -1) %>%
    distinct() %>%
    arrange(speaker2) %>%
    pull(speaker)

keras_preds <- dense_model %>%
    predict(prepped_training) %>%
    as_tibble() 

colnames(keras_preds) <- colnames_keras 

colnames_keras2 <- tibble(pred_class = colnames_keras) %>%
    mutate(speaker_num = rep(1:8))


keras_preds <- keras_preds %>% 
    rownames_to_column("id") %>%
    bind_cols(candidates_train %>% select(speaker)) %>% 
    rowwise() %>%
    mutate(speaker_num = which.max(c_across(2:9))) %>%
    left_join(colnames_keras2) 
